...
export const signup = catchAsync(async (req, res, next) => {
  ...
  // 2) Generate the token and send response
...
// Authorization middleware to allow only certain roles to access certain routes
...
export const updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(new AppError("No user found wit that email.", 404));
  }
